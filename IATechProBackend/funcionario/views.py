...
def py_cria_funcionario(request):
    data = request.data
    ...
    return Response({'status': 'success', 'funcionario_id': funcionario_id}, status=201)

    @api_view(['DELETE'])
    @permission_classes([IsAuthenticated])
    def py_delete_funcionario(request, funcionario_id):
        """
    ...
    @api_view(['PUT'])
    @permission_classes([IsAuthenticated])
    def py_edita_funcionario(request, funcionario_id):
